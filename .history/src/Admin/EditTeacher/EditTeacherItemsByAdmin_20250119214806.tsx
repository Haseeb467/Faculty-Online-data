...
import { collection, doc, getDocs, query, where, QueryDocumentSnapshot } from "firebase/firestore";
...
const EditTeacherItemsByAdmin: React.FC<EditTeacherItemsByAdmin> = ({ name, department, email, teacherId, isactive }) => {
    ...
    const handleToggle = async () => {
        ...
        try {
            ...
            const updatePromises = userDocs.docs.map((userDoc: QueryDocumentSnapshot ) =>
    ...
    return (
        <tr
            className={` border-b ${!activeStatus ? 'bg-red-200  ' : 'bg - white'}`}
